...
var _danabrams$elm_media$Native_Media = function() {
    ...
    function withMediaNode(id, doStuff)
    {
        return _elm_lang$core$Native_Scheduler.nativeBinding(function(callback)
        {
            rAF(function()
            {
                ...
                {
                    callback(fail({ctor: 'NotMediaElement', _0: id, _1: media.constructor.name}));
                    return;
                }
    ...
    function canPlayType(id, type){
        return withMediaNode(id, function(media) {
            switch (media.canPlayType(type)){
                ...
                default:
                    return fail({ ctor: "UndefinedCanPlayType", _0: type, _1: media.canPlayType(type)});
            }
        }); 
    }


    function toState(media){
        var element;
        if (media instanceof HTMLAudioElement){
            ...
            if (quality !== undefined) {
                elmQuality = {  ctor: 'Just',
                                    _0: {   creationTime: quality.creationTime,
                ...
                elmQuality = { ctor: 'Nothing' };
            }
            var videoExtra = {  width: media.videoWidth,
                                height: media.videoHeight,
                                videoTracks: processVideoTracks(media.videoTracks), 
                                quality: elmQuality }
            element = { ctor: 'Video', _0: videoExtra };
        }
        return {
            element: element,
            id: media.id,
            audioTracks: processAudioTracks(media.audioTracks),
            buffered: processTimeRanges(media.buffered),
            currentSource: media.currentSrc,
            currentTime: media.currentTime,
            duration: media.duration,
            ended: media.ended,
            error: processError(media.error),
            loop: media.loop,
            networkState: processNetworkState(media.networkState),
            playback: figureOutPlayback(media),
            playbackRate: media.playbackRate,
            readyState: processReadyState(media.readyState),
            seekable: processTimeRanges(media.seekable),
            seeking: media.seeking,
            volume: processVolume(media)
        };
    }

    function figureOutPlayback(media){
        if (media.paused) {
            if (media.error !== null){
                return {ctor: 'Error', _0: processError(media.error)};
            } else if (media.readyState !== 4) {
                if (media.readyState == 0 || media.readyState == 1){
                    return {ctor: 'Buffering'};
                } else if (media.readyState == 2 || media.readyState == 3){
                    return {ctor: 'Loading'};
                }
            } else if(media.ended){
                return {ctor: 'Ended'};
    ...
    function processVolume(media){
        if (media.muted){
            return {ctor: 'Muted'};
        } else {
            return {ctor: 'Volume', _0: media.volume}
        }
    }

    function processNetworkState(networkState){
        switch (networkState) {
            case 0:
                return { ctor: 'Empty'};
            case 1:
    ...
    function processError(error){
        ...
        switch (error.code) {
            case 1:
                return just({ ctor: 'Aborted', _0: error.message});
            case 2:
                return just({ ctor: 'Network', _0: error.message});
            case 3:
                return just({ ctor: 'Decode', _0: error.message});
            case 4:
                return just({ ctor: 'Unsupported', _0: error.message});
        }
    }

    function processReadyState(readyState){
        switch (readyState) {
            case 0:
                return { ctor: 'HaveNothing'};
            case 1:
                return { ctor: 'HaveMetadata'};
            case 2:
                return { ctor: 'HaveCurrentData'};
            case 3:
                return { ctor: 'HaveFutureData'};
            case 4:
                return { ctor: 'HaveEnoughData'};
        }
    }

    function processTimeRanges(ranges){
    ...
    function processAudioTracks(tracks){
        var list = _elm_lang$core$Native_List.Nil;
        for (var i = tracks.length; i--; )
        {
            var track = tracks[i]
            var audioTrack = {  enabled: track.enabled, 
                                id: track.id,
                                kind: track.kind,
                                label: track.label,
                                language: track.language };
            list = _elm_lang$core$Native_List.Cons(audioTrack, list);
        }
        return list;
    }

    function processVideoTracks(tracks){
        if (tracks !== null) {
            var list = _elm_lang$core$Native_List.Nil;
            for (var i = tracks.length; i--; )
            {
                var track = tracks[i]
                var videoTrack = {  id: track.id,
                                    kind: track.kind,
                                    label: track.label,
                                    language: track.language };
                list = _elm_lang$core$Native_List.Cons(videoTrack, list);
            }
            return {ctor: Just, _0: {list: list, selected: list[tracks.selected]}};
        } else {
            return {ctor: Nothing};
        }
    }
    ...
    function decodeState(value){
        ...
        {
            return _elm_lang$core$Result$Err({ ctor: 'NotFound', _0: value.name });
        } else if ((!(value.target instanceof HTMLMediaElement)))
        {
            return _elm_lang$core$Result$Err({ctor: 'NotMediaElement', _0: id, _1: value.target.constructor.name});
        }
        return _elm_lang$core$Result$Ok(toState(value.target));
    }

    function decodeTimeRange(value){
        if (value instanceof TimeRanges)
        {
            var list = _elm_lang$core$Native_List.Nil;
            for (var i = value.length; i--; )
			{
				var timeRange = { start: value.start(i), end: value.end(i) };
				list = _elm_lang$core$Native_List.Cons(timeRange, list);
			}
			return _elm_lang$core$Result$Ok(list);
        }
        else
        {
            return _elm_lang$core$Result$Err("Value is not an instance of TimeRanges");
        }
    }
...
var _danabrams$elm_media$Media$subscribe = function (id) {
	return _elm_lang$core$Native_Utils.crash(
		'Media',
		{
			start: {line: 348, column: 5},
			end: {line: 348, column: 16}
		})('Not Implemented Yet');
};
var _danabrams$elm_media$Media$canPlayType = F2(
...
var _danabrams$elm_media$Media$getState = _danabrams$elm_media$Native_Media.getState;
var _danabrams$elm_media$Media$currentTime = function (time) {
	return A2(
		_elm_lang$html$Html_Attributes$property,
		'currentTime',
...
var _danabrams$elm_media$Media$State = function (a) {
	return function (b) {
		return function (c) {
			return function (d) {
				return function (e) {
												...
												return function (m) {
													return function (n) {
														return function (o) {
															return function (p) {
																return function (q) {
																	return {element: a, id: b, audioTracks: c, buffered: d, controls: e, currentSource: f, currentTime: g, duration: h, loop: i, networkState: j, playback: k, playbackRate: l, played: m, readyState: n, seekable: o, seeking: p, volume: q};
																};
															};
														};
													};
...
var _danabrams$elm_media$Media$VideoExtra = F4(
	function (a, b, c, d) {
		return {height: a, width: b, quality: c, tracks: d};
	});
var _danabrams$elm_media$Media$VideoPlaybackQuality = F4(
	function (a, b, c, d) {
		return {creationTime: a, totalFrames: b, dropped: c, corrupted: d};
	});
var _danabrams$elm_media$Media$TimeRange = F2(
	function (a, b) {
		return {start: a, end: b};
	});
var _danabrams$elm_media$Media$AudioTrack = F5(
	function (a, b, c, d, e) {
		return {enabled: a, id: b, kind: c, label: d, language: e};
	});
var _danabrams$elm_media$Media$VideoTrack = F5(
	function (a, b, c, d, e) {
		return {selected: a, id: b, kind: c, label: d, language: e};
	});
var _danabrams$elm_media$Media$VideoTrackList = F2(
	function (a, b) {
		return {list: a, selected: b};
	});
var _danabrams$elm_media$Media$Video = function (a) {
	return {ctor: 'Video', _0: a};
};
var _danabrams$elm_media$Media$Audio = {ctor: 'Audio'};
var _danabrams$elm_media$Media$Muted = {ctor: 'Muted'};
var _danabrams$elm_media$Media$Volume = function (a) {
	return {ctor: 'Volume', _0: a};
};
var _danabrams$elm_media$Media$NoSource = {ctor: 'NoSource'};
var _danabrams$elm_media$Media$DataLoading = {ctor: 'DataLoading'};
var _danabrams$elm_media$Media$Idle = {ctor: 'Idle'};
var _danabrams$elm_media$Media$Empty = {ctor: 'Empty'};
var _danabrams$elm_media$Media$HaveEnoughData = {ctor: 'HaveEnoughData'};
var _danabrams$elm_media$Media$HaveFutureData = {ctor: 'HaveFutureData'};
var _danabrams$elm_media$Media$HaveCurrentData = {ctor: 'HaveCurrentData'};
var _danabrams$elm_media$Media$HaveMetadata = {ctor: 'HaveMetadata'};
var _danabrams$elm_media$Media$HaveNothing = {ctor: 'HaveNothing'};
var _danabrams$elm_media$Media$Unsupported = function (a) {
	return {ctor: 'Unsupported', _0: a};
};
var _danabrams$elm_media$Media$Decode = function (a) {
	return {ctor: 'Decode', _0: a};
};
var _danabrams$elm_media$Media$Network = function (a) {
	return {ctor: 'Network', _0: a};
};
var _danabrams$elm_media$Media$Aborted = function (a) {
	return {ctor: 'Aborted', _0: a};
};
var _danabrams$elm_media$Media$PlayPromiseFailure = function (a) {
	return {ctor: 'PlayPromiseFailure', _0: a};
};
var _danabrams$elm_media$Media$UndefinedCanPlayTypeResult = F2(
	function (a, b) {
		return {ctor: 'UndefinedCanPlayTypeResult', _0: a, _1: b};
	});
var _danabrams$elm_media$Media$NotMediaElement = F2(
	function (a, b) {
		return {ctor: 'NotMediaElement', _0: a, _1: b};
	});
var _danabrams$elm_media$Media$NotFound = function (a) {
...
var _danabrams$elm_media$Media$Error = function (a) {
	return {ctor: 'Error', _0: a};
};
var _danabrams$elm_media$Media$Ended = {ctor: 'Ended'};
var _danabrams$elm_media$Media$Loading = {ctor: 'Loading'};
var _danabrams$elm_media$Media$Buffering = {ctor: 'Buffering'};
var _danabrams$elm_media$Media$Paused = {ctor: 'Paused'};
var _danabrams$elm_media$Media$Playing = {ctor: 'Playing'};

var _danabrams$elm_media$Media_Events$targetId = A2(
	_elm_lang$core$Json_Decode$at,
	{
		ctor: '::',
		_0: 'target',
...
var _danabrams$elm_media$Media_Events$onDurationChange = function (tagger) {
	return A2(
		_elm_lang$html$Html_Events$on,
		'durationchange',
		A2(_elm_lang$core$Json_Decode$map, tagger, _danabrams$elm_media$Media_Events$targetId));
};
...
var _danabrams$elm_media$Main$source = function (mediaSource) {
	var _p0 = mediaSource;
	return A2(
		...
		{
			ctor: '::',
			_0: _elm_lang$html$Html$text('Your browser does not support playback of the MP3 format. Please try another browser, such as Chrome.'),
			_1: {ctor: '[]'}
		});
};
var _danabrams$elm_media$Main$processError = function (error) {
	var _p1 = error;
	if (_p1.ctor === 'Nothing') {
		return '';
	} else {
		switch (_p1._0.ctor) {
			case 'NotFound':
				return A2(_elm_lang$core$Debug$log, 'Not Found: ', _p1._0._0);
			case 'NotMediaElement':
				return A2(
					...
					A2(
						...
						A2(
							...
							A2(
								_elm_lang$core$Basics_ops['++'],
								'\' is an instance of \'',
								A2(_elm_lang$core$Basics_ops['++'], _p1._0._1, '')))));
			case 'UndefinedCanPlayTypeResult':
				return A2(
					_elm_lang$core$Debug$log,
					'Received an undefined canPlayType() result: ',
					A2(
						_elm_lang$core$Basics_ops['++'],
						'media.canPlayType(',
						A2(
							...
							A2(_elm_lang$core$Basics_ops['++'], ') = ', _p1._0._1))));
			default:
				return A2(_elm_lang$core$Debug$log, 'Media.play() promise failed with the following message: ', _p1._0._0);
		}
	}
};
var _danabrams$elm_media$Main$PlayerState = F6(
	function (a, b, c, d, e, f) {
		return {id: a, currentTime: b, duration: c, source: d, error: e, video: f};
	});
var _danabrams$elm_media$Main$MP3 = function (a) {
...
var _danabrams$elm_media$Main$init = {
	...
	_0: {
		...
		currentTime: 0,
		duration: 0,
		source: _danabrams$elm_media$Main$MP3('https://audio.simplecast.com/3652010f.mp3'),
		error: _elm_lang$core$Maybe$Nothing,
		video: {width: 0, height: 0, quality: _elm_lang$core$Maybe$Nothing, tracks: _elm_lang$core$Maybe$Nothing}
	},
	_1: _elm_lang$core$Platform_Cmd$none
};
var _danabrams$elm_media$Main$Frame = function (a) {
	return {ctor: 'Frame', _0: a};
};
var _danabrams$elm_media$Main$MediaUpdate = function (a) {
			...
			case 'Frame':
				return {
					...
					_1: A2(
						...
						_danabrams$elm_media$Main$MediaUpdate,
						_danabrams$elm_media$Media$getState(model.id))
				};
			case 'MediaEvent':
				return {
					ctor: '_Tuple2',
					...
					_1: A2(
						_elm_lang$core$Task$attempt,
						_danabrams$elm_media$Main$MediaUpdate,
						_danabrams$elm_media$Media$getState(_p2._0))
				};
			default:
				if (_p2._0.ctor === 'Ok') {
					var _p4 = _p2._0._0;
					...
					return {
						...
						_0: _elm_lang$core$Native_Utils.update(
							model,
							{currentTime: _p4.currentTime, duration: _p4.duration, error: _elm_lang$core$Maybe$Nothing, video: video}),
						_1: _elm_lang$core$Platform_Cmd$none
					};
				} else {
					return {
...
var _danabrams$elm_media$Main$MediaEvent = function (a) {
	return {ctor: 'MediaEvent', _0: a};
};
var _danabrams$elm_media$Main$audioPlayer = function (player) {
	var error = _danabrams$elm_media$Main$processError(player.error);
	return A2(
		_elm_lang$html$Html$audio,
		{
			ctor: '::',
			_0: _elm_lang$html$Html_Attributes$id(player.id),
			_1: {
				ctor: '::',
				...
				_1: {
					...
					_1: {
						ctor: '::',
						_0: _danabrams$elm_media$Media_Events$onTimeUpdate(_danabrams$elm_media$Main$MediaEvent),
						_1: {
							ctor: '::',
							_0: _danabrams$elm_media$Media$muted(false),
							_1: {ctor: '[]'}
						}
					}
				}
		...
		{
			ctor: '::',
			_0: _danabrams$elm_media$Main$source(player.source),
			_1: {ctor: '[]'}
		});
};
var _danabrams$elm_media$Main$view = function (model) {
	return A2(
		...
		{
			ctor: '::',
			_0: _danabrams$elm_media$Main$audioPlayer(model),
			_1: {
				ctor: '::',
				_0: A2(
					...
					{
						ctor: '::',
						_0: _elm_lang$html$Html_Attributes$id('luftballoon'),
						_1: {ctor: '[]'}
					},
					{
						ctor: '::',
...
var _danabrams$elm_media$Main$main = _elm_lang$html$Html$program(
	{
		view: _danabrams$elm_media$Main$view,
		init: _danabrams$elm_media$Main$init,
		update: _danabrams$elm_media$Main$update,
		subscriptions: _elm_lang$core$Basics$always(_elm_lang$core$Platform_Sub$none)
	})();
var _danabrams$elm_media$Main$videoPlayer = function (player) {
	var error = _danabrams$elm_media$Main$processError(player.error);
	return A2(
				...
				_1: {
					...
					_1: {
						...
						_1: {
							...
							_1: {
								ctor: '::',
								_0: _elm_lang$html$Html_Attributes$src('http://clips.vorwaerts-gmbh.de/VfE_html5.mp4'),
								_1: {ctor: '[]'}
							}
						}
					}
...
if (typeof _danabrams$elm_media$Main$main !== 'undefined') {
    _danabrams$elm_media$Main$main(Elm['Main'], 'Main', {"types":{"unions":{"Media.Volume":{"args":[],"tags":{"Muted":[],"Volume":["Float"]}},"Media.Error":{"args":[],"tags":{"NotFound":["Media.Id"],"NotMediaElement":["Media.Id","String"],"UndefinedCanPlayTypeResult":["String","String"],"PlayPromiseFailure":["String"]}},"Media.MediaError":{"args":[],"tags":{"Network":["String"],"Decode":["String"],"Unsupported":["String"],"Aborted":["String"]}},"Maybe.Maybe":{"args":["a"],"tags":{"Just":["a"],"Nothing":[]}},"Media.ElementType":{"args":[],"tags":{"Video":["Media.VideoExtra"],"Audio":[]}},"Main.Msg":{"args":[],"tags":{"MediaUpdate":["Result.Result Media.Error Media.State"],"Frame":["Time.Time"],"MediaEvent":["Media.Id"]}},"Media.Playback":{"args":[],"tags":{"Playing":[],"Error":["Media.MediaError"],"Loading":[],"Buffering":[],"Paused":[],"Ended":[]}},"Media.NetworkState":{"args":[],"tags":{"Idle":[],"Empty":[],"DataLoading":[],"NoSource":[]}},"Result.Result":{"args":["error","value"],"tags":{"Ok":["value"],"Err":["error"]}},"Media.ReadyState":{"args":[],"tags":{"HaveEnoughData":[],"HaveCurrentData":[],"HaveNothing":[],"HaveMetadata":[],"HaveFutureData":[]}}},"aliases":{"Media.State":{"args":[],"type":"{ element : Media.ElementType , id : Media.Id , audioTracks : Media.AudioTrackList , buffered : Media.TimeRanges , controls : Bool , currentSource : String , currentTime : Time.Time , duration : Time.Time , loop : Bool , networkState : Media.NetworkState , playback : Media.Playback , playbackRate : Float , played : Media.TimeRanges , readyState : Media.ReadyState , seekable : Media.TimeRanges , seeking : Bool , volume : Media.Volume }"},"Media.TimeRange":{"args":[],"type":"{ start : Time.Time, end : Time.Time }"},"Media.AudioTrack":{"args":[],"type":"{ enabled : Bool , id : String , kind : String , label : String , language : String }"},"Media.VideoTrackList":{"args":[],"type":"{ list : List Media.VideoTrack, selected : Media.VideoTrack }"},"Media.AudioTrackList":{"args":[],"type":"List Media.AudioTrack"},"Media.VideoPlaybackQuality":{"args":[],"type":"{ creationTime : Time.Time , totalFrames : Int , dropped : Int , corrupted : Int }"},"Media.VideoTrack":{"args":[],"type":"{ selected : Bool , id : String , kind : String , label : String , language : String }"},"Media.Id":{"args":[],"type":"String"},"Media.TimeRanges":{"args":[],"type":"List Media.TimeRange"},"Time.Time":{"args":[],"type":"Float"},"Media.VideoExtra":{"args":[],"type":"{ height : Int , width : Int , quality : Maybe.Maybe Media.VideoPlaybackQuality , tracks : Maybe.Maybe Media.VideoTrackList }"}},"message":"Main.Msg"},"versions":{"elm":"0.18.0"}});
}
